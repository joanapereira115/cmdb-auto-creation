...
# TODO: verificar que os pedidos são executados com sucesso!
            ...
            if "catg" in obj_categories_request.json()["result"]:
                for cat_g in obj_categories_request.json()["result"]["catg"]:
                    ...
                    if cat in cat_attr_types:
                        dialogs.update(cat_attr_types.get(cat).get("dialogs"))
...
def process_i_doit():
    ...
    print(json.dumps(
        cmdb_data_model["ci_dialog_attributes"], indent=4, sort_keys=True))
    return api_info