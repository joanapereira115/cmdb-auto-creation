    ...
    <xsl:template name="name_parser">
        ...
        <xsl:choose>
            ...
            <xsl:when test="matches($text, '[A-Z]')">
                <xsl:analyze-string regex="^([A-Z])" select="$text">
...
(  
    ...
    <xsl:template match="PROPERTY.REFERENCE">
        , <xsl:call-template name="name_parser"><xsl:with-param name="text" select="./@REFERENCECLASS" /></xsl:call-template>_id INTEGER REFERENCES <xsl:call-template name="name_parser"><xsl:with-param name="text" select="./@REFERENCECLASS" /></xsl:call-template>(id),
    </xsl:template>
